...
def main(): 
    ...
    pilot_sumrate = [0 for i in range(5)] 
    ...
    while t < sim_times:
        ...
        while itf_percent < 5:
            ...
            pilot_sumrate[itf_percent - 1] += round(sum(greedy_sumrate_i), 4)
    ...
    for i in range(len(opt_sumrate)):
        ...
        pilot_sumrate[i] /= sim_times
        freq_sumrate[i] /= sim_times
        opt_sumrate[i] *= 0.18 
        pilot_sumrate[i] *= 0.18 
        freq_sumrate[i] *= 0.18 
    ...
    plt.plot(x, pilot_sumrate, label = 'pilot', color = 'red', linewidth = 3 ,marker = 'o')
    plt.plot(x, freq_sumrate, label = 'freq', color = 'green', marker = 's')
